...
describe("balance model", function () {
  describe("getBestSnapshot", () => {
    it("should find snapshot", async function () {
      ...
      await book.entry("Test 1")
        .credit("Assets:Receivable", 1)
        .debit("Income:Rent", 1)
        .commit();

      await book.entry("Test 2")
        .debit("Assets:Receivable", 1)
        .credit("Income:Rent", 1)
        .commit();

      const balance1 = await book.balance({account: "Assets:Receivable"});

      expect(balance1).to.deep.equal({balance: 0, notes: 2, credit: 1, debit: 1});

      const snapshot = await getBestSnapshot({book: book.name, account: "Assets:Receivable"});

      expect(snapshot).to.have.property("balance", 0);
      expect(snapshot).to.have.property("credit", 1);
      expect(snapshot).to.have.property("debit", 1);
    });
    ...
    it("should return proper number of notes", async function () {
      ...
      const balance1 = await book.balance({account: "Assets:Receivable"});
      expect(balance1).to.deep.equal({balance: 1, notes: 1, credit: 1, debit: 0});

      const balance2 = await book.balance({account: "Assets:Receivable"});
      expect(balance2).to.deep.equal({balance: 1, notes: 1, credit: 1, debit: 0});

      await book.entry("Test 1").credit("Assets:Receivable", 1).debit("Income:Rent", 1).commit();

      const balance3 = await book.balance({account: "Assets:Receivable"});
      expect(balance3).to.deep.equal({balance: 2, notes: 2, credit: 2, debit: 0});
    });
    ...
    it("should not confuse snapshots", async function () {
      ...
      expect(balance).to.deep.equal({balance: 1, notes: 1, credit: 1, debit: 0});
      ...
      expect(balance).to.deep.equal({balance: 3, notes: 2, credit: 3, debit: 0});
    ...
    it("should snapshot with mongodb query language", async function () {
      ...
      expect(balance2).to.deep.equal({balance: 300, notes: 1, credit: 1, debit: 0});
    });
    ...
    it("should ignore the order of doc insertion", async function () {
      ...
      const balance1 = await book.balance({account: "Assets:Receivable"});
      expect(balance1).to.deep.equal({balance: 2, notes: 2, credit: 2, debit: 0});
      ...
      const balance2 = await book.balance({account: "Assets:Receivable"});
      expect(balance2).to.deep.equal({balance: 2, notes: 2, credit: 2, debit: 0});
    });
  });
});
